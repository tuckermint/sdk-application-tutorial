...
正如你在[应用设计](./01-app-design.md)章节中看到的，你的tuckermint需要一组模块：`auth`，`bank`，`staking`，`distribution`，`slashing`和`tuckermint`。前两个已经存在了，但最后一个还没有！`tuckermint`模块将定义你的状态机的大部分内容。下一步是构建它。
