...
Start by navigating to `./x/tuckermint/internal/types/querier.go` file. This is where you will define your querier types.

<<< @/tuckermint/x/tuckermint/internal/types/querier.go

## Querier

Now you can navigate to the `./x/tuckermint/internal/keeper/querier.go` file. This is the place to define which queries against application state users will be able to make. Your `tuckermint` module will expose three queries:
...
func NewQuerier(keeper Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		default:
			return nil, sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, "unknown tuckermint query endpoint")
		}
	}
}
```
...
- The type Whois is not defined in the `./x/tuckermint/internal/types/querier.go` file because it is created in the `./x/tuckermint/internal/types/types.go` file.
