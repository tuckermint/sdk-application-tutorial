...
To start your SDK module, define your `tuckermint.Keeper` in the `./x/tuckermint/internal/keeper/keeper.go` file. Defined in this generated file are a few extra items that we will not cover at this time, for this reason we will start by clearing the `keeper.go` file in favor of following this tutorial.
...
The last piece of code needed in the `./x/tuckermint/internal/keeper/keeper.go` file is a constructor function for `Keeper`:
