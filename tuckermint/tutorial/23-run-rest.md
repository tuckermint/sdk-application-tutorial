...
Now that you tested your CLI queries and transactions, time to test same things in the REST server. Leave the `tmd` that you had running earlier and start by gathering your addresses:

```bash
$ tmcli keys show jack --address
$ tmcli keys show alice --address
```

Now its time to start the `rest-server` in another terminal window:

```bash
$ tmcli rest-server --chain-id namechain --trust-node
```
...
$ curl -s http://localhost:1317/auth/accounts/$(tmcli keys show jack -a)
...
$ curl -s http://localhost:1317/auth/accounts/$(tmcli keys show alice -a)
...
curl -XPOST -s http://localhost:1317/tuckermint/names --data-binary '{"base_req":{"from":"'$(tmcli keys show jack -a)'","chain_id":"namechain"},"name":"jack1.id","amount":"5nametoken","buyer":"'$(tmcli keys show jack -a)'"}' > unsignedTx.json
...
tmcli tx sign unsignedTx.json --from jack --offline --chain-id namechain --sequence 1 --account-number 0 > signedTx.json

# And finally broadcast the signed transaction
tmcli tx broadcast signedTx.json
...
$ curl -XPUT -s http://localhost:1317/tuckermint/names --data-binary '{"base_req":{"from":"'$(tmcli keys show jack -a)'","chain_id":"namechain"},"name":"jack1.id","value":"8.8.4.4","owner":"'$(tmcli keys show jack -a)'"}' > unsignedTx.json
...
tmcli tx sign unsignedTx.json --from jack --offline --chain-id namechain --sequence 2 --account-number 0 > signedTx.json
tmcli tx broadcast signedTx.json

# Query the value for the name jack just set
$ curl -s http://localhost:1317/tuckermint/names/jack1.id
# 8.8.4.4

# Query whois for the name jack just bought
$ curl -s http://localhost:1317/tuckermint/names/jack1.id/whois
# > {"value":"8.8.8.8","owner":"cosmos127qa40nmq56hu27ae263zvfk3ey0tkapwk0gq6","price":[{"denom":"STAKE","amount":"10"}]}

# Alice buys name from jack
$ curl -XPOST -s http://localhost:1317/tuckermint/names --data-binary '{"base_req":{"from":"'$(tmcli keys show alice -a)'","chain_id":"namechain"},"name":"jack1.id","amount":"10nametoken","buyer":"'$(tmcli keys show alice -a)'"}' > unsignedTx.json

# Again we need to sign and broadcast
# NOTE: The account number has changed to 1 and the sequence is now 2, according to the query of alice's account
tmcli tx sign unsignedTx.json --from alice --offline --chain-id namechain --sequence 2 --account-number 1 > signedTx.json
tmcli tx broadcast signedTx.json
...
$ curl -XDELETE -s http://localhost:1317/tuckermint/names --data-binary '{"base_req":{"from":"'$(tmcli keys show alice -a)'","chain_id":"namechain"},"name":"jack1.id","owner":"'$(tmcli keys show alice -a)'"}' > unsignedTx.json
...
tmcli tx sign unsignedTx.json --from alice --offline --chain-id namechain --sequence 3 --account-number 1 > signedTx.json
tmcli tx broadcast signedTx.json
...
#### `POST /tuckermint/names` BuyName Request Body:
```json
{
  "base_req": {
    "name": "string",
...
#### `PUT /tuckermint/names` SetName Request Body:
```json
{
  "base_req": {
    "name": "string",
...
#### `DELETE /tuckermint/names` DeleteName Request Body:
```json
{
  "base_req": {
    "name": "string",
